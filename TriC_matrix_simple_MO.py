...
def do_plot(ax, Z):
    ...
    label_for_plot = str(chrom) + ":" + str(start) + "-" + str(stop) + "\nBin size " + str(
        bin_size) + "b  :  Max value " + str(threshold) + " RPM/bin"
...
def print_help(exitcode):
    ...
    print(
        "\t\t\t\t This file can contain ONLY intra-chromosomal (i.e. cis) fragments (as is the default behavior of TriC_MO.pl).")
    print(
        "\t\t\t\t Any inter-chromosomal (i.e trans) fragments in the file will cause the output matrix to be wrongly counted.\n")
    ...
    print(
        "-t --threshold \t optional \t Max value of normalised interactions for a bin - values above this will be capped to value [threshold] (default 20 RPM/bin)")
    print("\t\t\t\t The RPM bin-wise normalisation follows formula (for interactions between bins i,j) :")
    print(
        "\t\t\t\t (10^6/all_interactions_of_whole_matrix)*(sum_of_interactions_for_bin_ij/(REfragments_in_i+REfragments_in_j)\n")
...
unixOptions = "hvac:vl:vr:vb:vs:vo:vf:vt:vnl:vnr:vnc:v"
gnuOptions = ["help", "afmhot", "viridis", "chr=", "str=",
              "stp=", "bin=", "sample=", "outdir=", "file=", "threshold=",
              "nstr=", "nstp=", "normchrom="]
...
for currentArgument, currentValue in arguments:
    ...
    elif currentArgument in ('-nc', '--normchrom'):
        if currentValue:
            normchrom = currentValue
    elif currentArgument in ('-nl', '--nstr'):
        if currentValue:
            normstart = int(currentValue)
    elif currentArgument in ('-nr', '--nstp'):
        if currentValue:
            normstop = int(currentValue)
...
def read_input(full_file_name_ery, start, stop):
...
np.clip(matrix, 0, 20)
np.savetxt(full_file_name_out_ery + "_" + str(bin_size) + "_" + str(threshold) + ".tab", matrix, delimiter='\t',
           fmt='%2.2f')
